...
from metabasepy.exceptions import MetabaseConfigError


@pytest.mark.parametrize(
    "endpoint,username,password,token",
    [
        (None, None, None, None),
        ("", "", "", ""),
        ("http://metabase.com", "", "", ""),
...
def test_config_success(endpoint, username, password, token):
    client = MetabaseClient(endpoint, username=username, password=password, token=token)
